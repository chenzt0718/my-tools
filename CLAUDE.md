...
- 用户输入在展示前做 XSS 过滤
- 加密使用 Fernet (AES-128-CBC + HMAC)，密钥通过环境变量 `ENCRYPTION_SECRET` 配置
...
# 安装依赖
pip install flask openpyxl pandas xlrd cryptography
```
...
- **Markdown 笔记** (`notes-app/`) — Flask + 原生 JS 的本地笔记应用，支持实时预览
...
**加密模块 (`crypto_stub.py`)** — 使用 Fernet (AES-128-CBC + HMAC-SHA256) 进行真实加解密：
- 密钥通过 SHA-256 派生自主密钥 + 文件路径，主密钥从 `ENCRYPTION_SECRET` 环境变量读取
...
**后端 (`server.py`)** — 单文件 Flask 应用，文件系统存储（`notes/` 目录）：
- 笔记以 `.md` 文件存储在 `notes/` 目录
- 安全：路径校验防止目录穿越（`os.path.normpath` + 前缀检查）
- API：`GET/POST /api/notes`、`GET/PUT/DELETE /api/notes/<filename>`

**前端 (`templates/index.html`)** — 纯原生 JS，暗色主题：
- 左侧边栏：笔记列表 + 新建按钮
- 右侧分栏：Markdown 编辑器 + 实时预览（marked.js CDN）
- 自动保存（800ms 防抖）、Ctrl+S 手动保存、Ctrl+N 新建
- 状态栏显示当前文件和字数
...
- 加密功能已替换为真实 AES 加密（Fernet），不再使用 base64 挡板
